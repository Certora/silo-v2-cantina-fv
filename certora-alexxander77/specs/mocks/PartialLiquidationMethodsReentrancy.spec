...
methods {
    ...
    // MOCKED summaries
    function SiloConfig.turnOnReentrancyProtection() external => turnOnReentrancyProtectionMock();
    function SiloConfig.turnOffReentrancyProtection() external => turnOffReentrancyProtectionMock();

    function SiloConfig.getConfigsForSolvency(address _borrower) external returns (ISiloConfig.ConfigData, ISiloConfig.ConfigData) =>
...
function getExactLiqMock() returns (uint256, uint256, uint256, bytes4) {
    ...
    errorCodeCached = a;
    require(z != 0); // repayDebtAsset should not be zero
    return (x, y, z, a);
}
...
function initBalanceFlags() {
    ...
    balanceUpdateFlagS0 = false;
    balanceUpdateFlagS1 = false;

    balanceUpdateFlagP0 = false;
    balanceUpdateFlagP1 = false;

    balanceUpdateFlagD0 = false;
    balanceUpdateFlagD1 = false;
}
function nonSceneAddressRequirements(address sender) {

    require sender != Harness;
...
persistent ghost bool balanceUpdateFlagS0;
persistent ghost bool balanceUpdateFlagS1;

persistent ghost bool balanceUpdateFlagP0;
persistent ghost bool balanceUpdateFlagP1;

persistent ghost bool balanceUpdateFlagD0;
persistent ghost bool balanceUpdateFlagD1;
...
hook Sstore Silo0.erc20Storage._balances[KEY address account] uint256 newBalance (uint256 previousBalance) {
    balanceUpdateFlagS0 = true;
}

hook Sstore Silo1.erc20Storage._balances[KEY address account] uint256 newBalance (uint256 previousBalance) {
    balanceUpdateFlagS1 = true;
}

hook Sstore ShareProtectedCollateralToken0.erc20Storage._balances[KEY address account] uint256 newBalance (uint256 previousBalance) {
    balanceUpdateFlagP0 = true;
}

hook Sstore ShareProtectedCollateralToken1.erc20Storage._balances[KEY address account] uint256 newBalance (uint256 previousBalance) {
    balanceUpdateFlagP1 = true;
}

hook Sstore ShareDebtToken0.erc20Storage._balances[KEY address account] uint256 newBalance (uint256 previousBalance) {
    balanceUpdateFlagD0 = true;
}

hook Sstore ShareDebtToken1.erc20Storage._balances[KEY address account] uint256 newBalance (uint256 previousBalance) {
    balanceUpdateFlagD1 = true;
}
...
function requireTokenInScene(address token) {
    require
    token == Token0 ||
    token == Token0 ||
    token == Silo0 ||
    token == Silo1 ||
    token == ShareProtectedCollateralToken0 ||
    token == ShareProtectedCollateralToken1 ||
    token == ShareDebtToken0 ||
    token == ShareDebtToken1;
}