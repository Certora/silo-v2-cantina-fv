...
function isWithInterest(env e) returns bool {
    ...
    uint256 debt = silo0.getTotalAssetsStorage(ISilo.AssetType.Debt);

    return siloIRTimestamp != 0 && siloIRTimestamp < e.block.timestamp && debt != 0;
}

// function requireCorrectSilo0Balance() {
//     mathint collateralAssets = silo0.getTotalAssetsStorage(ISilo.AssetType.Collateral);
//     mathint protectedAssets = silo0.getTotalAssetsStorage(ISilo.AssetType.Protected);
//     mathint debtAssets = silo0.getTotalAssetsStorage(ISilo.AssetType.Debt);
...
//     mathint collateralAssets = silo1.getTotalAssetsStorage(ISilo.AssetType.Collateral);
//     mathint protectedAssets = silo1.getTotalAssetsStorage(ISilo.AssetType.Protected);
//     mathint debtAssets = silo1.getTotalAssetsStorage(ISilo.AssetType.Debt);
//     mathint daoAndDeployerFees = getSiloDataDaoAndDeployerFees();
