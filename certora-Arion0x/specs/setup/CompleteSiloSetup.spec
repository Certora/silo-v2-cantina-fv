...
definition HARNESS_METHODS(method f) returns bool = 
    f.selector == sig:Silo0.getSiloDataInterestRateTimestamp().selector
    || f.selector == sig:Silo0.getSiloDataDaoAndDeployerRevenue().selector
    || f.selector == sig:Silo0.getFlashloanFee0().selector
    || f.selector == sig:Silo0.reentrancyGuardEntered().selector
    || f.selector == sig:Silo0.getDaoFee().selector
    || f.selector == sig:Silo0.getDeployerFee().selector
    || f.selector == sig:Silo0.getLTV(address).selector
    || f.selector == sig:Silo0.isBelowMaxLTVHarness(ISiloConfig.ConfigData memory,ISiloConfig.ConfigData memory, address,ISilo.AccrueInterestInMemory).selector
    || f.selector == sig:Silo0.getConfigsForSolvencyHarness(address).selector
    || f.selector == sig:Silo0.getConfigsForWithdrawHarness(address).selector
    || f.selector == sig:Silo0.getDebtConfigTokenBalance(ISiloConfig.ConfigData memory, address).selector
    || f.selector == sig:Silo0.getAssetsDataForLtvCalculations(address).selector
    || f.selector == sig:Silo0.getTransferWithChecks().selector
    || f.selector == sig:Silo0.getSiloFromStorage().selector
    || f.selector == sig:Silo0.getConfig().selector
    || f.selector == sig:Silo0.setSiloAsCollateral(address).selector
    || f.selector == sig:Silo0.getFlashFee(address,address,uint256).selector
    ;

definition HARNESS_VIEW_METHODS(method f) returns bool 
    = HARNESS_METHODS(f) || f.isView;

definition REQUIRE_SOLVENCY_WITHDRAW_METHODS(method f) returns bool =
    f.selector == sig:Silo0.withdraw(uint256,address,address).selector ||
    f.selector == sig:Silo0.withdraw(uint256,address,address,ISilo.CollateralType).selector ||
    f.selector == sig:Silo0.transitionCollateral(uint256,address,ISilo.CollateralType).selector; 

definition UNNECESSARY_MINT_BORROW_REPAY_REDEEM_WITHDRAW_DEPOSIT(method f) returns bool =
    f.selector == sig:Silo0.mint(uint256,address,ISilo.CollateralType).selector ||
    f.selector == sig:Silo0.borrowShares(uint256,address,address).selector ||
    f.selector == sig:Silo0.borrowSameAsset(uint256,address,address).selector ||
    f.selector == sig:Silo0.repay(uint256,address).selector ||
