...
definition onlySiloContractsMethods(method f) returns bool =
    ( (f.contract == shareDebtToken0 || f.contract == shareDebtToken1 ||
        ...
        (   f.selector == sig:ShareDebtToken0.synchronizeHooks(uint24,uint24).selector ||
            ...
            // f.selector == sig:ShareDebtToken0.forwardTransferFromNoChecks(address,address,uint256).selector || this is not onlySilo function
    ...
    (  f.contract ==  siloConfig &&
        (   f.selector == sig:SiloConfigHarness.onDebtTransfer(address,address).selector ||
            f.selector == sig:SiloConfigHarness.turnOnReentrancyProtection().selector ||
            f.selector == sig:SiloConfigHarness.turnOffReentrancyProtection().selector ||
            f.selector == sig:SiloConfigHarness.setOtherSiloAsCollateralSilo(address).selector ||
            f.selector == sig:SiloConfigHarness.setThisSiloAsCollateralSilo(address).selector
        )
     )
     ;
...
rule check_sanity(method f) filtered { f -> !onlySiloContractsMethods(f) && f.selector != sig:ShareDebtToken0.decimals().selector && f.selector != sig:Silo0.initialize(address).selector /*because the initialze function will revert cause the config alreadey existed*/ } 
{
    // setup requirements 
    env e;
    configForEightTokensSetupRequirements();
